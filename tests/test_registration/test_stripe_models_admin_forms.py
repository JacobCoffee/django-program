...
@pytest.mark.django_db
def test_stripe_customer_id_can_repeat_across_conferences(conference: Conference, conference_alt: Conference, user):
    StripeCustomer.objects.create(
        user=user,
        conference=conference,
...
@pytest.mark.django_db
def test_stripe_customer_admin_has_no_delete_permission(site, request_obj):
    admin_cls = StripeCustomerAdmin(StripeCustomer, site)
    assert admin_cls.has_delete_permission(request_obj) is False
    assert admin_cls.has_delete_permission(request_obj, obj=None) is False
...
@pytest.mark.django_db
def test_stripe_event_admin_has_no_delete_permission(site, request_obj):
    admin_cls = StripeEventAdmin(StripeEvent, site)
    assert admin_cls.has_delete_permission(request_obj) is False
    assert admin_cls.has_delete_permission(request_obj, obj=None) is False
...
@pytest.mark.django_db
def test_event_processing_exception_admin_has_no_delete_permission(site, request_obj):
    admin_cls = EventProcessingExceptionAdmin(EventProcessingException, site)
    assert admin_cls.has_delete_permission(request_obj) is False
    assert admin_cls.has_delete_permission(request_obj, obj=None) is False
