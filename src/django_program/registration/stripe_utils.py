...
def convert_amount_for_api(amount: Decimal, currency: str) -> int:
    ...
    if currency.upper() in ZERO_DECIMAL_CURRENCIES:
        if amount != amount.quantize(_ONE_WHOLE):
            msg = f"Amount {amount} is invalid for zero-decimal currency {currency}."
            raise ValueError(msg)
        return int(amount)

    if amount != amount.quantize(_ONE_CENT):
        msg = f"Amount {amount} has more than 2 decimal places for currency {currency}."
        raise ValueError(msg)
